...
<div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1> Certify Internal Requisition</h1>
          </div>
          <div class="col-sm-6">
 ...
 <div class="form-group row">
          ...
          <br>
            ...
            <!-- /.card -->
                        ...
                        @if(in_array(auth()->user()->role_id,[1,2,10,11,12,13]))
                        <button type="button"   class="btn btn-primary float-right"  onclick="Certify('{{$internalRequisition->id}}');">Certify</button></br>
                       @else
                       <button type="button"   class="btn btn-primary float-right"   disabled>Certify</button></br>
                       @endif
...
function Certify(internal_requisition_id){
        ...
        if (isConfirm) {
          console.log("approve");
          $.post( {!! json_encode(url('/')) !!} + "/certify-internal-requisition",{ _method: "POST",data:{internal_requisition_id:internal_requisition_id,permission:1},_token: "{{ csrf_token() }}"}).then(function (data) {
          console.log(data);
            if (data == "success") {
              swal(
                "Done!",
  ...
  // console.log(internal_requisition_id);
   swal({
        ...
        if (isConfirm) {
          ...
          $.post( {!! json_encode(url('/')) !!} + "/certify-internal-requisition",{ _method: "POST",data:{internal_requisition_id:internal_requisition_id,permission:0,comment:comment},_token: "{{ csrf_token() }}"}).then(function (data) {
          console.log(data);
            if (data == "success") {
              swal(
                "Done!",
                ...
                location.href='/certify-internal-requisition';
